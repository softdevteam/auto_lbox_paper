...
the outer language. Language boxes are shrunk to encompass less content if they
contain a syntax error and moving the content to the outer language does not
...
ensure that it pretends the language box of its starting node does not exist.
When there is a change \laurie{near/after?} or inside an uncommitted language
...
from that which matches the most content and working backwards, we then check
whether the conditions above are satisfied and, as soon as we match one, we
resize the language box appropriately.

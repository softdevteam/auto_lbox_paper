...
\caption{A generic candidates recogniser which produces the ending offsets of
  ...
  EOS (End Of String) token, we assume that further input cannot fix the parse
    and terminate the search (lines 12--13).\lukas{this algorithm currently suggests that we reparse
    every substring from the beginning, which isn't necessary, since we can just continue from the previous
    parsing state after receiving the next token from the lexer. the \texttt{add\_before\_eos} also seems
    to suggest that we have some kind of tree that the recogniser parses. however, the recogniser is just a bog-standard
    batch-parser that just takes the next token and processes it using the parsing table. to do the eos check, we just virtually parse
    an eos token to see if the parser reaches an accept state. if it doesn't we continue from the state of the previously parsed token.}}
...
failed earlier than the EOS token, then we assume\lukas{we actually know, not just assume} that no further input will
...
use of incremental parsing's ability \laurie{actually, i'm not sure what it's
doing! are we resetting everything in the parse tree to a previous version?
just one version back?}\lukas{It is using history of the parse tree. The
...
is the beginning of the subtree)} . This approach

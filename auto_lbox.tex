...
this paper we show that language boxes -- a delimiter-based
...
disambiguation operators can filter all possible parse forests into a single
parse tree.
...
opposite of the situation with generalised parsing where language composition
...
updates or creates tokens as necessary, and marks path from each updated or
...
locations, and that it relies on the structure of LR grammars, which does not
always match human expectations of a language's structure.
...
the candidates from the individual heuristics, ranks them, and removes
all of the non-best candidates.

In essence, our ranking mechanism orders
candidates by how far we can parse successfully after them without encountering
...
much input subsequent tokens consume, precisely defining ``how far'' is
surprisingly difficult. As a reasonable proxy, we use the absolute offset in
the input in characters as the precise ordering mechanism. To bound the
performance cost of this check, we check a fixed number (currently 10)
non-whitespace tokens forward from the inserted language box: if
one or more candidates are successful up to that point they outrank all
candidates which have syntax errors within that token range.
\lukas{There are a few things that a missing now that were explained in the now
deleted filtering heuristics section. One is that we automatically insert a box
if there is only one candidate and let the user choose if there are more. The
other is the thing about whitespace, which is still acurrate. While we now
parse further than the first non-whitespace, we still don't accept language
boxes that can only parse a single white-space token.}
...
\end{picture}
\vspace{-1.2em}
\caption{\laurie{i think this example is now irrelevant: ranking means that we naturally don't discard this language box. please check that i'm correct :)} An example showing why parsing too far beyond a candidate language box increases
...
the language box can simply reappear on each subsequent repress, which is
unlikely to de desired. Therefore removing a language box in this way marks
...
\begin{enumerate}
  ...
  \item If an uncommitted language box becomes part of a syntax error in the outer
    ...
    language box should be removed.
  \item If an uncommitted language box's content is valid in both the inner and
    ...
    the language box as a proxy for this\laurie{is this/should this be still true?}.
\end{enumerate}
...
the first non-whitespace token in the outer language \laurie{again, is this/should this be true?}.
...
\begin{table*}[tb]
    ...
    \vspace{7pt}
    \caption{\lukas{Explain what corpus size means, or maybe change it to "test size"?}\laurie{How about `\#{}tests'?}The total percentage of acceptable outcomes for each benchmark and
...
outer language, that means that \laurie{can we make a macro which is equivalent
to breakdownallvalidsame*(100.0/(100-breakdownallnovalid) i.e. 88.3\% or so
with the current values} of cases insert a language box around the fragment --
...
in the outer language) and no insertions due to multiple candidates are roughly
equal in size.
...
our individual candidates heuristics. Unsurprisingly, the \texttt{lines}
candidates heuristic is the most effective, because it correctly deals with
nearly all insertions of expression fragments. The main exception is the
...
(e.g.~`function x() \{') looks like a Java function with a return type
\texttt{function}: it is thus parsed as a Java function which leads to syntax
errors on subsequent lines of PHP code. We expected the parse tree candidates
heuristic to be more effective than the stack candidates heuristic, but the
latter (\validstackoverall) has a noticeable greater proportion of acceptable
outcomes than the former (\validhistoverall). \laurie{any idea why? i don't
understand why it does so badly on both javalua and phplua -- is it something
to do with lua's syntactic structure?}\lukas{I looked into the logs and its a combination
of various things: first, an often chosen location for an insertion is inside brackets, e.g.~\texttt{if (here)}.
This means that the fragment is inserted immediately after the opening bracket, so that the first fragment
token and the bracket share the same parent, so that the parse tree heuristic can't find the valid location
after the bracket. However, this applies to all compositions with outer languages PHP/Java. But Lua has another
unfortunate property. Many of the fragments look something like this \texttt{\{a,b\}}, which means they cause
an error immediately when inserted. Whereas a PHP fragement inside Java, for example, often starts with an identifier, giving
the incremental parser a chance to reparse the tree before the rest of the fragment is inserted. This reparse
allows the tree to be rewritten just enough, so that the first fragment token and the bracket don't share the same
immediate parent anymore, allowing the heuristic to find this location. Finally, some Lua fragments have the \texttt{//}
problem we talk about in 4.6, which adds to the amount of undetected insertions.} The stack candidates heuristic, however,
...
it is written in Python, a relatively slow language; and Eco's data-structures
...
additional undo history). Despite that, performance cost is generally
acceptable: the average per-keypress wall-clock time of incremental parsing (including
everything associated with automatic language boxes) was approximately 0.015s,
with only 1.6\% of tests experiencing an average wall-clock time above 0.1s
(roughly the threshold at which humans start to perceive some lag from typing)
and a worst case of 2.8s. A more efficient implementation would probably reduce
these costs by at least one order of magnitude, if not more.
...
\begin{acks}
This research was funded by \emph{redacted for double blind review}.
%the EPSRC Lecture (EP/L02344X/1) fellowship.
\end{acks}
...
\laurie{is it possible to use the same order of language compositions that we do in table 1 columns (i.e. alphabetic)?}
\begin{table*}[b]

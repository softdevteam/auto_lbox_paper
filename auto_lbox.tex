...
(\breakdownallvalidsame) leading to a language box being inserted
around the entire fragment, and most of the rest (\breakdownallvaliddiff)\lukas{I wasn't sure if you meant partial insertions or noinsertions here? The text suggested the former, but the comment the latter.}
...
expression contained several subexpressions. For PHP and Java \laurie{why not lua too?}\lukas{The extracted lua expressions didn't suffer as much from the same program, so I left them as is. This can be changed tho.}
...
\begin{table*}[tb]
    ...
    \vspace{7pt}
    \caption{\lukas{Explain what corpus size means, or maybe change it to "test size"?}The total percentage of acceptable outcomes for each benchmark and

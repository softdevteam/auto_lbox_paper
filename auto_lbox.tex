...
ways that would surprise users if they were aware of it.\lukas{there are two reasons we use history:
1) subtrees on the current parsing stack may be detached from the root, so traversing parents
up to the root is not always possible}\laurie{can you explain this a bit more for me? detached in what way? i thought everything was attached, but perhaps attched to the wrong node? clearly i'm wrong in some way!}
\lukas{if we create a new nonterminal node (and we couldn't reuse an old one),
this node doesn't have a parent yet. but because of (ambiguous) node reuse one
of the error nodes ancestors can end up as a child of that new nonterminal. so
when then try to traverse the error nodes ancestors we end up in the new nonterminal
node and won't be able to continue traversing up to the root.}
We thus use the
...
current value).
Since we store
previous parsing states onto the nodes in the parse tree \laurie{eh, this seems
a comically inefficient use of memory! surely we wouldn't do this in
production?}\lukas{it's not really that much more inefficient compared to the other stuff
we store on the nodes. i think wagners original algorithm doesn't store states
on nodes, but storing them helps us a lot with other features, e.g. showing in the
editor which language boxes are valid at the current cursor position, or in this case
checking if a language box can be inserted without having to reparse the parse
tree to that point. so, we don't really need to do this, but without some of these algorithms
perferm a bit worse. That being said, given we can use incremental parsing for most of them,
it might not be a huge performance loss, but that's hard to say without trying. Though if we
want to optimise memory use storing only the diffs of changed tokens values might be a better target.}\laurie{hang on, do we store the parsing stack or just a single parsing state? if it's the latter, fine (it's just a machine word at worse).}\lukas{ah, haha, I see the misunderstanding now. We only store the single state where the node was shifted onto the stack. if we'd store the entire stack that would indeed be laughably inefficient.}, we can just use the
...
outer program is being reparsed.\lukas{this is similar to my comment in the expanding section.
we currently need to check these every time the program is reparsed which is not very efficient.
an alternative approach could be to only check these boxes if the surrounding area has changed, though
that itself is hard to determine, e.g.~how large should be make this surrounding area? the easiest
would be to only recheck these boxes if they are encountered by the parser (i.e.~not skipped) but
that may not be enough.}\laurie{yes, but it's deeply unclear to me what ``every time the program is reparsed'' means: do we have to inspect the whole tree or ...?}
\lukas{ah, sorry. the program is incrementally reparsed every time the user makes an edit (e.g. after a keypress). though we don't have to inspect the whole tree
to find these language boxes (they are all stored in a list and easily accessible). But since its hard to tell which language box the latest edit by the user
may affect, we just check all of them each time.}
...
the outer language\lukas{is it worth reminind the user that we only check
the first non-whitespace token?}. Language boxes are shrunk to encompass less content if they
...
Fortunately both growing and shrinking can be handled with a small variation to
our existing candidates recogniser (see Section~\ref{sec:recognisers}) to
\lukas{I don't understand the rest of this sentence}ensure that it pretends the language box of its starting node does not exist.
When there is a change \laurie{near/after?}\lukas{this is tricky to define,
...
expansion is checked everytime an edit happens anywhere in the program} or inside
...
from the candidate which matches the most content and working backwards, we then check
whether the conditions above are satisfied and, as soon as we match one, we
resize the language box appropriately.\lukas{this is also not entirely true. we only
expand a box if there is a single candidate. otherwise we let the user pick similar to
insertion. for shrinking we currently only try to parse everything up to the error node.
it hasn't actually occured to me to run the recogniser on the whole language box to
see if there may be other shrinking candidates (probably because I incrementally
parsed straight to the error node, which is faster then reparsing the entire language box
again)}

...
inserted terminals to be detached from the tree~\cite[p.~60]{wagner98practicalalgorithms}\lukas{It's not directly spelled out here but ``the input is a mixture of old subtrees from the previous version of the tree'' somewhat implies it}\laurie{is there somewhere in your thesis that explains it?}. Fortunately, we can solve this
...
we reparse all nodes up to the language box (lines 10--17), skipping
...
def parse_tree(parser, node):
  ...
  while node is not None:
    ...
    node = node.get_parent(global_version - 1)
  return cnds
...
walks up the parse tree to find candidate language boxes (line 14--15).
...
began (line 16) -- \texttt{node.get\_parent(\textit{V})} returns
\texttt{node}'s parent as when \texttt{node} was at in version \texttt{V} (which may be different
to \texttt{node}'s parent in \texttt{global\_version}). The \texttt{line} heuristic searches
backwards each token from the error node (line 25) until the beginning of
the line which contains that node (line 21) for
candidate language boxes (lines 23--24).}

...
performance cost of this check, we check a fixed number (currently 10) of
...
A specific weakness of the parse tree heuristic can be seen in the JavaLua
and PHPLua compositions. Lua's table\footnote{Often called associative arrays,
hashmaps, or dictionaries in other languages.} constructors have the syntax
`\texttt{\{e1, ..., en\}}'. As soon as the initial `\texttt{\{}' is
inserted, a Java/PHP syntax error is detected and, often, the parse tree
rewritten in such a way that the associated token \lukas{This reparse
allows the tree to be rewritten just enough, so that the first fragment
token and the bracket don't share the same immediate parent anymore,
allowing the heuristic to find this location.}\laurie{isn't the bracket the
first fragment token?} The stack candidates heuristic, however,

...
we found that these more often contain somewhat interesting expressions. For Lua, however,
the case was almost reverse: since the Lua test suite uses many \qtt{assert}s, their parameters
contained the most interesting expressions in some files. We thus extracted all expressions from
those Lua files, including those from function calls.
...
outliers.}
\lukas{Here are the results per keypress: 2.8s (Longest keypress), 0.0008 (Shortest keypress), 0.015 (Average keypress), 46 (No of keypresses greater 0.1s), 1.6\% (Percentage of keypresses greater 0.1s)}

...
location using that language's lexer and parser \laurie{this is a recogniser, right? so can we talk a bit about how it works? because it's not actually doing stuff with the parse tree or ... ?}. It stops consuming text when
...
\caption{An example showing how using the first token, or first non-whitespace
  ...
  is a candidate for the same reason. `1 or 2 and 3;` is a candidate, because it following is a newline
    ...
    insertions, the first one will always be immediately inserted after it has been typed, so we will never get multiple options this way.}\laurie{if that's the case, i think we should just delete this figure. it's going to confuse more than it enlightens.}
The

...
mechanism atop incremental parsing -- can be extended in a way that places it
...
boxes in many useful cases. We quickly found that manipulating arbitrary language
boxes is surprising and irritating. Instead we use a simple model to capture
the notion of ``I'm not finished typing here'', where automatically inserted language boxes
start as \emph{uncommitted} (meaning they can still be updated, removed, or resized) and
\emph{committed} (which means the heuristic treats them as explicit insertions by the user). Language boxes
...
Tokens have a mutable type \laurie{is it true that nonterminals have an immutable type but terminals have a mutable type?}\lukas{yes} (e.g.~`int')
and a mutable value (e.g.~`3') but cannot have children at all. Figure \ref{fig_example_parsetree}

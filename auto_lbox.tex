...
certain situations nodes can end up disconnected from the root \laurie{we need to cite a page or pages from wagner's thesis that backs this up, because otherwise we're going to have to explain it ourselves, which is a scary thought}.
\lukas{This is going to be difficult. I don't think he mentions anything along
those lines. The closest I could find is this when he talks about ambiguous node reuse: ``Consider what happens if two
children of a node both exist in the new version of the tree but with different
parents -- which, if either, should be the reuse site?  The policy we adopt in
our implementation is first-come/first-served.''. From here we can argue,
that the second of the two node is the error node, which hasn't been parsed yet,
and thus is still attached to its old parent. But the old parent has already been
reused and attached to a new nonterminal (which is detached). Thus there is no path
from the error node to the root.}\laurie{we need to do something here: on the basis that, after you, i probably understand wagner's algorithm as well as nearly anyone, i don't have a clue what the problem is, or why our solution works. we've either got to explain it fully, or reference something that explains it. i'll be honest and say that i'm baffled that nodes can become detached: it runs counter to my intuitions about how wagner's algorithm works. is this connected to the fact that parsing increments the versions by two?}
\lukas{I may have not been very clear about the detaching thing. Nodes can
become detached, but only briefly during parsing. As soon as parsing finishes
everything is back in order.  Just think about an empty parse tree into which
we insert a single token. Let's take the familiar calculator grammar as an
example. We insert 1 which then gets reparsed. The parser shifts `1` onto the
stack. Then `1` gets reduced to `P` for which we create a new nonterminal
(since there are no nonterminals to reuse yet) and push that onto the stack.
This nonterminal `P` has no parent yet. This means that `1` which is a child of
`P` is now detached from the root of the parse tree (note, that we can still
traverse down to `1` from the root using the previous parse tree -- whenever
the incremental parser reparses a parse tree, it always needs to traverse the
previous version since we are actively changing stuff around during parsing,
kindof like iterating over a list that we add/remove elements from).  Now we
continue parsing, which applys the reductions `T->P` and `E->T`, which attaches
P to T, and T to E, however, E is still detached. Then the parser reaches an
accept state, which means there should only be a single node left on the tree
which is pretty much our entire new parse tree (intertwined from new and old
nonterminals). We then just take this node from the top of the stack and whack
inbetween BOS and EOS and we're done.  This is how subtrees in general can
become detached. However, this doesn't explain how the error node can become
attached, since it hasn't even been parsed yet, which means it couldn't have
been reduced and attached to a new nonterminal. This is were node reuse comes
into play. Whenever a reduction happens, we see if we can reuse an old node
which already exists in the tree.  There are two variants of node reuse.
Unambiguous node reuse can only reuse an old nonterminal, if the new children
and the old children are all the same, which is a bit too strict. Which is why
there is another variant: ambiguous node reuse. There only one of the children
needs to be the same. Let's now assume we have two nodes, `1` and `e`, which
are both attached to a nonterminal `A`. The node `e` was just inserted and
needs to be reparsed. Let's then assume `1` gets reduced using a rule `A->1`,
so we can reuse the previous `A`. However, due to the `e` being there things
are being parsed differently, and we use a rule `X->A`, but no nonterminal `X`
exists yet. So we create a new nonterminal `X` and attach `A` to it. `X` is not
detached. However, `e` which now causes an error still has a parent pointer to
`A` (since we haven't parsed `e` yet), and thus be definition `e` is now also
detached from the root. Of course then error recovery kicks and and isolates
trees and reverts them and so on, so at the end of the parse no detached
subtrees exist. However, since the candidate heuristic gets triggered at the
point of the error, the parse tree is still broken, and thus when traversing
the tree upwards from the error node, we need to use the previous version of
the parse tree.  }
We thus use the
...
current value). \laurie{why does putting things back one version guarantee that we don't have disconnected nodes?}\lukas{because
after every parse we have an intact parse tree. It may contain isolated errors, but there can't be any detached nonterminals
since parsing has completed and all reductions have been applied. if we go one version back we get a (at some point) reparsed
parse tree which only had tokens added in. we only get detached parse trees in the middle of the parse. we only store
parse trees once parsing has finished, so there are never any detached parse trees in the history.}
...
valid parse stack for candidates from the history and line heuristics.
Fortunately, we can do so by using incremental parsing \laurie{i think we mean
to say ``we can do so by using the state of the incremental
parser''?}\lukas{no, we are basically reparsing the entire parse tree again all
the way up to the candidate location where we want to insert the language box.
however, because anything subtree that isn't a direct ancestor of that location
can just be skipped (i.e.~optimistically shifted).  This is equivalent to
changing the node at the candidate location as changed, and then reparsing the
parse tree all the way to that node.}
to generate the parse stack without revisiting the entire tree, by skipping
subtrees are not on a direct path to the candidate location.
This way we can recreate the parsing stack as if we had inserted
the language box at that point and reparsed the changed parse tree (see Figure
\ref{fig_preparse} \laurie{this figure doesn't seem obviously related
...
def history(parser, node):
  cnds = []
  node = node.version(global_version - 1)
...
starting point of language boxes (using the candidates recogniser from
    Figure~\ref{fig:recogniser}) \laurie{what is the intuition behind \texttt{next\_lookahead}?\lukas{this used to be called poplookahead by Wagner, but in my thesis
    I changed this to nextlookahead. It simply returns the next node in the parse tree (similar to a depth-first traversal)}}. The \texttt{stack} heuristic walks the parse
...
walks up (line 17) a previous version of the parse tree (line 12) for
candidate language boxes (line 15--16). The \texttt{line} heuristic searches
backwards each token from the error node back (line 26) until the beginning of
the line which contains that node (line 22) for
candidate language boxes (lines 24--25).}
...
language boxes as uncommitted, the only uncommitted language boxes\lukas{is it box or boxes?} can be at
...
either or both of\lukas{I think the two are mutually exclusive}: those token types which can start a
...
recogniser so that later stages don't have to do needless work?}\lukas{correct!}
...
file (whether it is inside or outside a language box) to be relexed. \laurie{I
have no idea what this sentence means}\lukas{It basically means,
since we are only pretend-lexing things for the recogniser, this has no
impact other than the performance hit.} However, since the lexing only happens
virtually and is not applied to the input (unless this result in the insertion
of a new language box), this does not affect the parse tree of the outer
language. And since lexing is generally quite fast, the performance impact will
also be insignificant.


\subsection{Wrong detection}

\laurie{does this ever trigger for anything except the java/python example?}
\lukas{It should also trigger for JavaSQL and probably does for many other compositions, e.g.}

\begin{lstdefault}
int y;
int x = SELECT * FROM t WHERE
y = 2; // Old Java code
\end{lstdefault}

\lukas{If we keep this subsection, I will update the example accordingly.}
...
\laurie{i'm not sure what to do with this section. do we need it?}
\lukas{probably not. I think we've explained most of this in prose and
the algorithm isn't all that exciting, if we can get the prose explanation right (which
currently needs a bit more tweaking)}
...
To evaluate the efficacy of automatic language boxes, a large-scale evaluation
...
(Java \laurie{7 or 8?}, Lua \laurie{5.3?}, PHP \laurie{what version?}, and
SQLite \laurie{what version?}) and created 12 language compositions from them.
...
function definitions and expressions (respectively) are valid \laurie{this
makes sense for java/lua/php, but what is a function definition in SQL?}.  The
particular language compositions we created can be seen as the $x$-axis labels
in Figure~\ref{tbl_valid}.

We then collected a corpus of inputs in each language. \laurie{we
need facts and figures here. at a minimum: where did we get the source code?
how many files were there? roughly how many lines?} We then ran each program
through a parser to identify the positions and spans of function definitions
and expressions. \laurie{how many of each did we find? did we use every
possible position? did we use subexpressions (e.g.~in 
\texttt{int x = 2 + 3 * 4} did we cut out `2 + 3 * 4' as a single expression or
did we also cut out `2 + 3'?}
...
fragment}) \laurie{did we record which pairs so that other people can replay
exactly the same things we used?} and: loaded \emph{base file} into our
extension of Eco; moved the cursor to a function definition or expression (as
...
place. \laurie{did we only insert linearly? i thought we were also going to try
non-linear insertions?} For each pair we recorded \laurie{what did we record?}.

...
immediately break the surrounding context, ranks the remainder, and removes all
...
this would cause us to filter out all candidates when the user themselves has
...
specify the first non-whitespace token, rather than simple the first token, is
...
multiple candidates were found). If a language box was inserted, we also
recorded the length of the fragment and the length of the inserted language box.
...
outer language, that means that \breakdownallexclude of cases insert a language box around the fragment --
...
and PHPLua compositions. Many Lua fragments consists of Lua's tables\footnote{Often called associative arrays,
hashmaps, or dictionaries in other languages.} constructors, which have the syntax
`\texttt{\{e1, ..., en\}}'. As soon as the initial `\texttt{\{}' is
inserted, a Java/PHP syntax error is detected. However, since the initial token
was inserted immediately after another token, both share the same parent, but since
the valid insertion point is inbetween the two tokens, the parse tree heuristic cannot
find this location. This problem doesn't occur, if the fragement starts with a token
that is valid in the outer language, since this allows the incremental parser to reparse
the parse tree, before the remaining fragment is inserted, thus often rearraging the tree
in a way, that the preceding token and the inserted fragment don't share a parent anymore.
\lukas{This reparse
...
first fragment token?}\lukas{no, the fragement is inserted inbetween the two brackets. I've tried to rewrite things. See if that makes sense.} The stack candidates heuristic, however,
...
it is written in Python, a relatively slow language, and the GUI runs only
...
language boxes is rarely affected. To try and get some idea of what the
...
to be inserted and derived average keypress times from that: the average
per-keypress wall-clock time of incremental parsing (including
...
and a worst case of 2.8s. We believe that a more efficient implementation could

...
\textbf{a)} The user is currently in the process of editing an existing Java program and has
just deleted the expression after \qtt{remote}.
\textbf{b)} After inserting an SQL statement an error appears in line 3. However,
since the heuristic
found multiple options to resolve the error, no language box is automatically inserted.
The editor indicates that there are multiple solutions for an error by showing
a lightbulb on the left hand side.
\textbf{c)} Clicking on the lightbulb reveals a drop-down list of the
candidates the heuristic has found.
\textbf{d)} Clicking on one of the options inserts the language box. This is
treated as a manua insertion, marking the language box as committed, which

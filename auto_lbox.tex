...
def lang_pstack(lbox):
  ...
  while node is not lbox:
    # should this be "if node in path_to_lbox"?
    if node not in path_to_lbox:
      parser.parse(node)
      # we seem to enter an infinite loop at this point because
      # node isn't changed
    else:
      if node.children:
...
\end{lstdefault}
\caption{An algorithm for quickly creating a parse stack after a language box
has been inserted into the parse tree.}
...
valid parse stack for candidates from the history and parse tree heuristics which
requires reparsing the program. \laurie{ref \ref{fig:createparsestack}}
...
searching for candidate language boxes (line 7). The \texttt{parse\_tree} heuristic

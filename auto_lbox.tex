...
\lukas{this can only happen if during typing parts of the language box can be parsed
in the outer language, which then creates a separate subtree, and continuing to type
inserts tokens into another remote subtree. in those cases we can't really do anything, which
is why we added the line heuristic.}
...
up to the root is not always possible 2) the current parse tree may have reparsed
contents of the language box in weird ways so that we cannot find the beginning of the language box
anymore (which is somewhat decsribed by the previous sentence)} We thus use the
...
us to walk over the tree in a different way?}\lukas{we don't really unwind
anything. we just look at an old version of the tree. we use the version of the
tree just before reparsing started, so that we have an intact parse tree that
still contains all recent changes. it's 1 version back} of the tree.
Since we store
...
want to optimise memory use storing only the diffs of changed tokens values might be a better target.}, we can just use the
...
\begin{enumerate}
  ...
  \item If an uncommitted language box causes a syntax error in the outer
    ...
    outer language. \laurie{do we check the first non-whitespace token in this
        context?}\lukas{no, because if a language box is not valid at its
        current location that is quite a severe error, so checking if the content
        can parse outside is enough to warrant the removal (e.g. the user may not be
        finished typing the outer input yet, so the code may not be valid yet).}

...
\begin{figure}
    \includegraphics[width=0.25\textwidth]{images/auto_remove_insert_sql_java}
    \begin{picture}(0,0)
        \put(-145,77){\textcolor{black}{\textbf{(a)}}}
        \put(-145,58.7){\textcolor{black}{\textbf{(b)}}}
        \put(-145,40.6){\textcolor{black}{\textbf{(c)}}}
        \put(-145,22.5){\textcolor{black}{\textbf{(d)}}}
        \put(-145,4.5){\textcolor{black}{\textbf{(e)}}}
    \end{picture}
    \caption{Automatic language boxes in action with a composition of SQL
      and Java, where Java expressions can be used wherever SQL expressions
      are valid. The initial input is `\texttt{SELECT x FROM z;}', with the user
      putting their cursor immediately after `\texttt{x}' and typing `\texttt{,y\textasciicircum3}':
      language boxes (with a pink background) are inserted and removed as the
      user types.  \textbf{a)} The user starts with a valid SQL statement and
      puts the cursor immediately after \texttt{x}. \textbf{b)} After typing `,', the SQL input \qtt{SELECT x, FROM z} causes
    a syntax error. At this point, the automatic language box heuristic
    is used: by inserting a Java language box around \qtt{FROM} (a valid
    Java identifier), the overall program becomes syntactically correct (since
    a Java language box is syntactically valid after the `,`). \textbf{c)} After typing
    `\texttt{y}' the language box is removed again, since `\texttt{FROM}' is
    valid in the outer language again. \textbf{d)} Typing `\texttt{\textasciicircum3}' produces an error
    which is fixed by wrapping the expression `\texttt{y\textasciicircum3}' inside a Java
    language box.
}
\label{intro_example}
\end{figure}

...
rare\lukas{Reviewer: Is there a solution for this? A: A different could solve
this, e.g.~ a line heuristic that traverses two lines backwards, however, then
we'd also need a 3, 4, etc line heuristic. Where do we stop?}.
...
outer language, that means that \breakdownallexclude \lukas{Reviewer: Where does this number come from? A: Percentage of complete insertions when removing no insertion (valid).}of all possible cases
...
and a worst case of 0.1s. We believe that a more efficient implementation could

...
associated with automatic language boxes) was 0.006s. Only two cases (out of
\laurie{how many?}) had a per-keypress wall-clock time above 0.1s (often
considered the threshold at which humans start to perceive some lag from
typing), with a worst case of 0.17s. These times are good on their own, but we
believe that a more efficient implementation could reduce these costs by at

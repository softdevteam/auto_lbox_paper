...
to say ``we can do so by using the state of the incremental parser''?}
\laurie{the rest of this sentence is so terse that i can't understand it} to
skip all subtrees
...
Figure~\ref{fig:recogniser}) \laurie{what is the intuition behind \texttt{next\_lookahead}?}. The \texttt{stack} heuristic walks the parse
...
have no idea what this sentence means} However, since the lexing only happens

...
possibility of old syntax errors being considered a second time, unless
the error itself or the area surrounding it are edited. \laurie{i
clearly haven't quite got this explanation right, because otherwise figure 1
wouldn't work since we go through several syntax errors until we get a new
language box. so errors aren't isolated and ignored immediately i guess? or
does each new keypress make an old error back into a new error?} \lukas{in the
example in figure 1 the text following `name' is inserted into the isolated
subtree. Whenever an isolated subtree is changed, it needs to be reinspected
during a parse, which means the error is also reparsed, but since it still
causes a parsing error, the heuristic is also triggered again.}
...
boxs which do not lead to a syntax error at the next non-whitespace token
...
inserting any language boxes in the rest of the program}. In other
...
incomplete program. \lukas{I agree with everything in the following paragraph, except
that I am not sure what you mean by an empty whitespace token.}The reason that we specify that the first non-whitespace
token must not contain an error is that grammars for incremental parsing almost
always define whitespace as a token, and whitespace can nearly always be empty.
This means that the incremental parser nearly always inserts a (possibly empty)
whitespace token after every candidate language box, which is by definition
syntactically valid. We thus need to skip such tokens in order to get to a
...
pressing undo (conventionally \keys{Ctrl+Z}). \laurie{i think we do some sort
of marking to make sure that we don't insert the same language box again?}\lukas{indeed! when the user
undos a language box, the error node is marked an no language box is automatically inserted again.
though there is an improvement on my todo list, that allows other language boxes to be added, because
atm we block all of them, which is a bit too strict.}.

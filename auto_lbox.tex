...
boxes in many useful cases. Automatically inserting language boxes is
generally non-controversial, but we quickly found that a heuristic which
...
want it to be subject to the heuristic again. One of the main challenges with
...
\begin{figure*}
    ...
    \begin{picture}(0,0)
        \put(-255,103){\textcolor{black}{\textbf{(a)}}}
        \put(-108,103){\textcolor{black}{\textbf{(b)}}}
        \put(  35,103){\textcolor{black}{\textbf{(c)}}}
        \put(-255,51){\textcolor{black}{\textbf{(d)}}}
        \put(  35,51){\textcolor{black}{\textbf{(e)}}}
    \end{picture}
    \vspace{-1em}
    \caption{Automatic language boxes in action with a composition of Java and
    SQL, where SQL statements can be used wherever Java expressions are valid.
    As the user types, a language box (with a pink background) is inserted and
    expanded depending on the context of the surrounding area. \textbf{a)} The
    user has started typing an SQL statement which causes a Java syntax error
    on \qtt{min}. However, no language box is inserted since that would
    introduce a new error immediately after the box at \qtt{\}}.  \textbf{b)}
    After typing `,', the automatic language box heuristic inserts an SQL
    language box around \qtt{SELECT min(age)} (a valid SQL statement) which
    fixes the error at \qtt{min} without introducing a new one immediately
    after the box, suggesting this is a valid insertion (see Section
    \ref{sec:parse_after_lbox} for more information). \textbf{c)} The user
    continues typing of the SQL statement which introduces new Java syntax
    errors into the program. \textbf{d)} After typing \qtt{;}, the automatic
    language box heuristic expands the existing language box to encompass the
    entire SQL statement, makeing the overall program syntactically correct.
    \textbf{e)} The user finished writing the remaining Java code wihtout any
    new language boxes being automatically inserted.
}
\label{intro_example}
\end{figure*}
...
\caption{\laurie{we need to inkscape this example and at the same point
  make it show java + sql}An elided example of an SQL language box nested within an outer Python
...
The chief weakness of language boxes is that they must currently be inserted
...
general) pressing a special key combination to complete the language box. For
examples which use language boxes sparingly, this is merely slightly
irritating, but when users write programs which frequently move between the
various languages in a compositions, it becomes a significant usability
concern. Eco, an editor which supports language boxes~\cite{diekmann14eco},
slightly eases this problem by using information from the incremental parser to
narrow down the list of languages presented to those valid at the cursor,
though this is a mild palliative at best.

In this section, we present a novel solution to this problem -- automatic
language boxes -- which shows that it is possible, in many cases, to
automatically insert, remove, and resize language boxes. We start by
setting out some basic criteria for a solution, before describing how
automatic language boxes work in detail. We later validate the usefulness of
automatic language boxes in Section~\ref{sec:evaluation}.


\subsection{Criteria for a solution}

In an ideal world, we would be able to insert language boxes exactly where and
when needed --- and never where they are not wanted. This is impossible in the
general case, because language composition is really grammar composition in
disguise, and is thus subject to the same ambiguity problems as generalised
parsing (see Section~\ref{sec:scannerless}). Most importantly, this means that
there will be cases where there are multiple valid ways of splitting the same
input across different language boxes. On top of this hard constraint, we
assert that a realistic automatic language box solution must satisfy several
softer considerations.

First, a solution which requires language composition authors (i.e.~the people
who actually compose grammars, create code generators etc.) is unlikely to be
used widely and/or correctly. For example, it can be difficult to know whether
a non-LR grammar is ambiguous or not~\cite{vasudevan13detecting}: grammar
composition often magnifies such concerns. Language composition authors are
often experts in parts of the underlying meta-system, but it is not realistic
to expect them to be expert in every single part. Ideally, any automatic
...
For example, one simple way of achieving the overall aim would be to reparse
the complete file on every keypress, which would be noticeably slow for large
...
practical performance. \laurie{does wagner talk about the problem of relexing
the whole file when you e.g.~open a string?}

Third, a solution whose effects are unpredictable would probably be turned off
by users. Clearly (given the hard constraint above) users cannot expect perfect
outcomes all the time. But a sensible minimum is that it should be entirely
predictable when automatic language boxes are potentially inserted, removed, or
resized; and, ideally, largely predictable as to what the effects of such
actions are. Furthermore, false negatives (i.e.~if the system inserts, removes,

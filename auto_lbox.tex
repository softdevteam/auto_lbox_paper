...
parsing stack, at each point seeing at the associated node. This heuristic has
...
use of incremental parsing's ability \laurie{actually, i'm not sure what it's doing! are we resetting everything in the parse tree to a previous version? just one version back?} . This approach
...
approach, this heuristic also need to recreate the parse stack to confirm
...
def history(parser, node):
  ...
  # also, shouldn't we check node before checking its parent or ... ?

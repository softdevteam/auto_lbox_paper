...
Our experiments show that in \laurie{XXX\%} automatic language boxes achieve an
acceptable result by which we mean they either: inserted a single valid
language box (\laurie{XXX\%}); did not insert a language box because the
inserted fragment was valid in both languages (\laurie{XXX\%}); or an ambiguity
was found and there were multiple different ways of inserting language boxes
(\laurie{XXX\%}). As this suggests, the number of cases where automatic
...
input was put into the language box (\laurie{XXX\%}); or a language box wasn't
inserted when we would have expected it to be (\laurie{XXX\%}). Our fully

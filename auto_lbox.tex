...
non-whitespace tokens forward from the inserted language box. However, since
this would disadvantage candidates with earlier start and ending locations,
we need to make this bound dynamic. We first pick
the candidate that is likely to parse the furthest, which we approximate by using
the candidate whose end position has the largest absolute character offset.
We then check how far this candidate allows parsing to continue (bound to a maximum of 10 tokens),
and record the absolute offset in characters. For each remaining candidate, we
then check if it can parse equally as far (or further) than that. Candidates that
do not manage to reach this far, i.e.~have syntax errors within that range, are rejected.
...
with less than 1\% of tests experiencing an average per-keypress wall-clock time above 0.1s
(roughly the threshold at which humans start to perceive some lag from typing)
and a worst case of 0.1s \laurie{this doesn't make sense: we say that `less than' (which is a bit vague) of tests take more than 0.1s, and the worst case is 0.1s. that suggests that the worst case is better than some other cases?!}. We believe that a more efficient implementation could

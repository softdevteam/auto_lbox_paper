...
from the error node to the root.}
We thus use the
...
\lukas{How to recreate the parsing stack is explained at the end of \ref{sec:filtering}. However,
for the candidate heuristic we don't actually need to recreate the parsing stack. It is sufficient
to check if the parse table has an action for the language box at the given state. This doesn't
guarentee that the language box is fully valid at that point (e.g. the lbox could first cause a reduction
but in the following state results in an error). But since the filtering heuristic does this check properly by
recreating the stack and then parsing the language box (plus whatever comes after), it will
filter out those cases afterwards.}.

...
However, the chief weakness of language boxes is that they must currently be
inserted manually by the user -- this involves pressing a special key
combination, selecting the desired inner language from a list, typing the
content, and (in general) pressing a special key combination to complete the
language box. If users want to write programs which frequently move between the
various languages in a composition, this can quickly become tedious. The Eco
editor, which is an implementation of an editor with language
boxes~\cite{diekmann14eco}, slightly eases this problem by using information
from the incremental parser to narrow down the list of languages presented to
those valid at the cursor, though this is a mild palliative at best.

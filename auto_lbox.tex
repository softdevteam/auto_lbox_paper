...
(e.g.~\cite{visser97scannerless}). Each approach has different downsides: delimiters
are visually intrusive; syntax directed makes editing programs
...
(leading to only a small performance overhead\lukas{this depends on the language
inserted, since opening a PHP quote will lex the entire remaining program. Tho
this is not really the fault of the heuristic}). One of the main challenges is
...
combined, lead to an effective heuristic for many cases. \laurie{at this point
we need a nice figure which shows our solution in action}

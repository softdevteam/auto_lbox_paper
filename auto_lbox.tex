...
expression contained several subexpressions. Overall, we identified 2,474 \laurie{this is a really low total: less than 10 expressions per PHP file?}\lukas{this number only represents actual right-hand-side expressions. i.e.~it doesn't include expressions from function calls or array values, as these are typically only single values (e.g.~strings) and are thus not very interesting. also wordpress has quite some files which are essentially just function calls, so they push this number further down.}
...
tree are very heavy weight, without any of the optimisations described

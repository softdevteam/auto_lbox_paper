...
equal furthest: we then parse 10 further tokens beyond that language box and
use that as the absolute extent to which we will check other language boxes.
For each other candidate language boxes we then continue parsing until, and
unless, it reaches that extent: any candidate language box which hits an
unsuccessful parse point before that extent is discarded; any candidate
language box which parses successfully up to, or beyond, that extent are

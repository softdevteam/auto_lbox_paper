...
\begin{figure*}
    ...
    \end{picture}
    \vspace{-2.2em}
    \caption{An example of automatic language boxes in action with our
      extension to the Eco editor. In this case, the user is entering text in a
      ...
      around the SQL as, while that would make the SQL valid, it would cause a
      syntax error in the first Java token after the language box (`\texttt{,}').
      \textbf{b)} After typing `\texttt{,}' the automatic language box heuristic inserts
      an SQL language box since at least one further token of Java
      (`\texttt{,}') is syntactically valid. \textbf{c)} The user continues
      typing a (now incomplete) SQL statement which causes syntax errors in the

...
description of this algorithm, we start by considering the problem of language box
...
possible candidates heuristics, each of which, as we shall see in
Section~\ref{sec:evaluation}, has different strengths and weaknesses. We thus
combine several of these heuristics into an overall candidates heuristic.

The first candidates heuristic is the \emph{stack} approach. This heuristic
uses the parsing stack at the time of a parsing error to search the input
backwards and find a location at which a language box can be inserted. The
advantage of this heuristic is that the availability of the parsing stack makes
it easy to test found candidates during the filtering heuristics.  After
inserting a candidate language box and pushing it onto the stack, we can just
continue parsing as normal, while the other heuristics will need to recreate
the stack at the point of insertion. To find a valid location, this heuristic
simply starts popping elements of the parsing stack. Each time it then tries to
parse a candidate language box in the current parsing state. The heuristic
stops when it reaches the beginning of the stack. Since the stack is typically
small (as many elements frequently get reduced into a single one), this
heuristic skips the majority of locations, many of which are possibly valid,
...
\begin{figure}[tb]
  \begin{subfigure}{20pt}
    \vspace{-4.65cm}\caption{}%
  \end{subfigure}
  \includegraphics[scale=0.4]{images/whitespace1.png}\\
  \begin{subfigure}{20pt}
    \vspace{-3.1cm}\caption{}%
  \end{subfigure}
\includegraphics[scale=0.4]{images/whitespace2.png}
\caption{An example showing how using the first token, or first non-whitespace
  token, after a candidate language box changes our idea of what a sensible
  insertion might be. Both examples use the same language composition (Java and
  Python, where Python expressions are valid wherever Java expressions are) and
  the same input text. \laurie{is it necessary for them to have pasted it in?
  could they have typed it? i must admit, the more i look at the example, the
  less i understand how we end up with 3 candidates...}\lukas{Yes, it needs to
    be pasted as otherwise a box would be immediately inserted after \texttt{1 or 2} and the rest
    then gets typed into the box correctly. When pasting, we only
  get 3 candidates if we use whitespace tokens to confirm candidates. `1 or 2' is a candidate,
  because we can insert it and shift the whitespace following it, which confirms it. `1 or 2 and 3`
  is a candidate for the same reason. `1 or 2 and 3;` is a candidate, because it following is a newline
  which can also be shifted after the language box, confirming it.}The
...
boxes which do not lead to a syntax error at the next non-whitespace token
\laurie{do we care about a syntax error before the language box?}\lukas{no, otherwise
a syntax error at the beginning of the file would keep us from
inserting any language boxes in the rest of the program}\laurie{well, yes, but
we could check the first non-whitespace token before a language box for errors?
i don't know if this is a good idea or not, but it's an obvious question that
someone will ask!}\lukas{We don't need to do this, since the heuristic needs to check
if the language box can be pushed at the chosen location, which it only can if
the parser can parse to this point (while using error recovery to isolate errors on the way).
this means that the immediate area preceding the language box can not have any errors. for example,
the stack heuristic doesn this implicitly since we only use locations that can be found on the
stack, guarenteeing that any location we pick has no errors. the other heuristics recreate the stack,
so picking a location with an error would fail the recreating process and the location is discarded}. In other
...
incomplete program. We do not need to check the area preceding the language box, since
any location picked by the heuristics is prooven to be valid at that point (though, isolated errors may exists
before that). For example the stack
heuristic only chooses locations from the current parsing stack, while the other two heuristics recreate
the parse stack at the location in order to confirm the candidate.
The reason that we specify that the first non-whitespace

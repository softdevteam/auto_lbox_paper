...
generalised parsing: while this only works well on language compositions which
users can, and have, defined appropriate disambiguations, generalised parsing
requires no subsequent effort on the part of the end user.
...
boxes, while not losing their generality.
...
essence, we present an algorithm which can automatically insert, remove, and resize language
...
\end{description*}
\laurie{collate these into `acceptable' and `unacceptable'}
...
expression contained several subexpressions. Overall, we identified 2,474 \laurie{this is a really low total: less than 10 expressions per PHP file?}\lukas{this number only represents actual right-hand-side expressions. i.e.~it doesn't include expressions from function calls or array values, as these are typically only single values (e.g.~strings) and are thus not very interesting. also wordpress has quite some files which are essentially just function calls, so they push this number further down.}\laurie{we need to define carefully which expressions we considered interesting. i'm not really sure what `actual right-hand-side' expressions means, for example.}
...
a poor vehicle for evaluating them: it is written in Python, but only runs on
CPython, a relatively slow implementation; and Eco's choice of data-structures
was designed for flexibility, not efficiency. For example, nodes in Eco's parse
tree are very heavy weight, and do not include any of the optimisations described
in~\citet{wagner98incremental}. Despite that, \laurie{what i'd hope to say here
...
The following tables show for each heuristic, the outcomes per benchmark. The
x-axis shows the outcomes of the insertions, similar to
Table~\ref{tbl:breakdown}, while the y-axis shows the benchmarks used.

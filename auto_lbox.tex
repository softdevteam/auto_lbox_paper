...
\subsubsection{The stack, parse tree, and line heuristics}
...
The second candidates heuristic is the \emph{parse tree} approach which aims to
...
inserted terminals to be detached from the tree. Fortunately, we can solve this
easily by using the versioning feature of the incremental parsing algorithm
from~\cite{wagner98incremental} \laurie{cite the page in wagner} which allows
...
changed and the incremental parser runs \laurie{cite a page from wagner's
thesis}: we reparse all nodes up to the language box (lines 10--17), skipping
subtrees which can't be relevant to the parsing stack (lines 16--17). The
\texttt{next\_lookahead} function (line 17) returns the next node in the parse
tree in preorder.}
...
searching for candidate language boxes (line 7) starting at the first terminal
following that node (line 6). The \texttt{parse\_tree} heuristic
walks up the parse tree (line 12) for candidate language boxes (line 15--16).
Because the parse tree is, in a sense, only partially parsed at the point the
candidates heuristic is called, we view the parse tree as it was before parsing
began (line 12). The \texttt{line} heuristic searches
...
While there is a clear indicator for when a language box may need to be shrunk (e.g.~
when it contains an error), this is not the case for the expansion of language boxes.
On every parse we thus run our altered candidates recogniser at the start of
each uncommitted language box (luckily, unless the user has manually marked
language boxes as uncommitted, this should only be a single language box).
...
either or both of\lukas{I think the two are mutually exclusive}\laurie{not necessarily: i could say `accept every string beginning with < but exclude anything beginning with <escape'. This isn't a major point though}\lukas{not if we use token type, which we do. We thus can only
say, the start token of the language box needs to be in a given set, or must not be in given set.}: those token types which can start a
...
the rest of a file matches both language's lexing rules\lukas{not entirely true. all they need to
share is comment or string syntax, in which case an open comment/string will lex to the end of the file}. Second, lexing is a
...
For PHP, we used the sourcecode of Wordpress, which consists of 365 files that are
valid when parsed with our PHP grammar, and amount to 173368 LoC. We also
removed any occurence of HTML from each file, since such files would otherwise
be invalid in a pure PHP grammar.
For Java, we used the Java Standard Library 5 which consists of 6556 files and
424051 LoC.
For Lua, we used Lua's test suite, which consists of 32 files totalling 13990
LoC.
For SQL, we used SQLite's test suite, consisting of 1100 test files which
contain 27526 tests totalling 89211 LoC.
...
expression contained several subexpressions. Overall, we extracted 2474
expressions and 518 functions for PHP, 52687 expressions and 6743 functions for
Java, 4476 expressions and 230 functions for Lua, and 36017 statements for SQL.
...
exactly the same things we used?}\lukas{yes. though we store absolute paths to the files,
so the experiment requires some adjustments} and: loaded \emph{base file} into our
...
non-linear insertions?}\lukas{Unfortunately, I never got around implementing this.}
For each pair we recorded whether a language box was inserted or not, if the insertion
...
\emph{location} combination, which were recorded during the first run.
Note, that if any editor related bugs occured during a run, all results from that \emph{base file}
were discarded.

\subsection{Results}

We break the results of our experiment down in two ways: 

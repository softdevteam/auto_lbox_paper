...
(including the ability to take the user's previous input into consideration\lukas{i think this related to historic tokens, a section we removed })
...
inner language always embed entire SQL statements\laurie{this suggests that we replace `def f(): pass' with `SELECT * from name' which sounds very wrong?!}. The
...
\laurie{i suggest we rename these to something like: `Valid insertion',
`invalid insertion', `valid in outer', `missed' (maybe `ignored'?), `multiple'.
we also need to beef up their definitions.}
\begin{description*}
  \item[Valid insertion] A language box was automatically inserted around the inner fragment.
  \item[Invalid insertion] \laurie{``a language box was inserted but it did not fix the error or introduced new ones'' is both vague and confusing. i don't get it}

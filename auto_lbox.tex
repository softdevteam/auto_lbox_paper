...
repeatable experiment can be found at \url{https://github.com/softdevteam/auto\_lbox\_experiments} \laurie{this should be the archive link. probably we should include the version of eco we used there as well so that we don't need to have a link to any github repo (particularly because github probably isn't a long-term way of distributing materials)}
and our extensions to the Eco editor can be found at \url{http://github.com/softdevteam/eco}.
...
plausible nodes in the parse treeat which
...
that contains the error (note that the line heuristic does not distinguish
between normal newlines or those within multiline comments or strings, even if
the editor chooses to presented the latter as a single node). This heuristic
ensures that all
...
\laurie{is this where we describe the newish optimisation?}
...
with only 1 test experiencing an average per-keypress wall-clock time above 0.1s (0.15s)

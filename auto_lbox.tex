...
\title{Default disambiguation for language composition}
...
show that default disambiguation rules, which are inappropriate for
batch parsing, lead to a new point in the design space for online editing.
We extend language boxes -- a delimiter-based
algorithm atop incremental parsing -- in such a way that default disambiguation
rules can automatically insert, remove, or resize, language boxes, leading to
...
In this paper we show that default disambiguation rules (i.e.~disambiguation
rules applied equally to every language composition) provide a satisfying new
point in the design space. Readers can be forgiven for surprise at this
statement, since default disambiguation is clearly inappropriate in batch
parsing, where a single incorrect disambiguation will cause dire results.
However, when using an online parser an incorrect disambiguation can be
easily overriden by the user. The challenge then becomes to find default
disambiguation rules which rarely require the user to override them.
...
the input (measured in characters) as the ordering mechanism. To bound the
...
matches a human's intuition. Fortunately, such ambiguities are
rare.
...
outer language, that means that \breakdownallexclude of cases insert a language box around the whole fragment --
...
in the outer language) and no insertions due to multiple candidates roughly
equal in proportion.
...
with less than 1\% of tests experiencing an average wall-clock time above 0.1s
(roughly the threshold at which humans start to perceive some lag from typing)
which was also the worst case. We believe that a more efficient implementation could

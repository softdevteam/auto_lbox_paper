...
\begin{abstract}
Since composed grammars are often ambiguous, grammar composition requires an
approach for dealing with ambiguity: either ruling it out entirely through
delimiters, or providing static disambiguation operators to select the desired
parse. In this paper we show that grammar composition based on incremental
parsing using the `language boxes' approach can be extended to explore the grey
area in-between these two extremes. In essence, we retain the use of
delimiters, but selectively reparse input to discover when the user has
switched from using one grammar in a composition to another, intelligently
inserting and updating delimiters. By definition this cannot always achieve a
perfect result, but our experiments show that it works surprisingly well.
\laurie{can we put a figure on this? i think this would need a baseline to
compare against though...}
\end{abstract}
...
Language composition offers an enticing solution for problems such as the
development of domain-specific languages and the migration of legacy software.
Unfortunately, approaches thus far have failed to meet expectations, largely
due to the difficulty of writing composed programs. The main difficulty is to
detect when the program has moved to or from an embedded language. This problem
manifests in different ways, depending on which parsing approach is used. For
example, composing two LR grammars together may result in a non-LR grammar,
which means we don't know what language the resulting grammar expresses. While
generalised parsing can parse any context-free grammar, including ambiguous
ones, the ambiguities we get from parsing composed programs lead to so many
parse trees, that it becomes difficult to find the one parse that the user
intended. One solution to these problems are separators between languages
(e.g.~\cite{wyk07context,batory98jts}). Unfortunately, this comes at the cost
of expressivity, as separators must not appear in any of the composed
languages. An alterntive to these approaches, is to move to an online solution,
which tries to automatically detect the boundaries between languages and falls
back on the user when this fails. The basis for this approach is incremental
parsing~\cite{wagner98practicalalgorithms} and language
boxes~\cite{diekmann14eco}.

\lukas{The following two or three paragraphs can probably be removed
or merged into the Background section.}

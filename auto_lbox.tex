...
Our experiments show that in \validalloverall automatic language boxes achieve an
...
\subsubsection{The stack, history, and line-based heuristics}
...
The first candidates heuristic is the \emph{stack} approach, which is based on the idea that each point in the parsing stack naturally defines a plausible breaking point between one language and another. It walks backwards over the parsing stack, at each point seeing if a language box
can be successfully inserted at that point. This heuristic has two significant advantages

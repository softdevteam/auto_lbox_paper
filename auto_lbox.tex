...
valid parse stack for candidates from the history and line heuristics which
requires reparsing the program. Fortunately, the incremental parser makes this
a cheap operation, because any subtree that isn't a direct ancestor of the
candidate language box will be optimistically shifted (i.e.~skipped).
\laurie{(see Figure
\ref{fig_preparse} this figure doesn't seem obviously related
...
starting point of language boxes (using the candidates recogniser from
    ...
    I changed this to nextlookahead. It simply returns the next node in the parse tree (similar to a depth-first traversal)}} \laurie{sinc we later define `prev\_terminal' can we use `next\_terminal' in the algorithm and explain that it's called poplookahead in wagner's thesis and nextlookahead in yours? that way the algorithm will be more intuitive for readers}. The \texttt{stack} heuristic walks the parse
...
either or both of\lukas{I think the two are mutually exclusive}\laurie{not necessarily: i could say `accept every string beginning with < but exclude anything beginning with <escape'. This isn't a major point though}: those token types which can start a
...
a candidate language box. Any such hints given by language composition authors
...
for incremental parsing \laurie{cite the page range in your thesis that shows
this}. However, this is not applicable to our situation, where candidates
...
if an inner language shares sufficient lexing rules with an outer language that
the rest of a file matches both language's lexing rules. Second, lexing is a
fast activity in general, and particularly fast in the recogniser because we
are not altering the parse tree in any way.
...
\laurie{aha, i like that example: it makes more sense to me now}
...
\end{lstdefault}
\end{minipage}
\caption{\laurie{do we need this figure?}A parse tree with a language box that we want to automatically remove (left)

...
boxes in many useful cases. The heuristic to insert a language box is triggered
only when there is a syntax error in the input of the language the user is
currently typing in (making it fairly predictable): we then run a recogniser
which attempts to insert language boxes to see if that leads to a successful
parse(s). However, further input can change our idea of what language boxes
should be inserted. We thus use a simple model of ``I'm not finished typing
yet''. Automatically inserted language boxes are initially considered
\emph{uncommitted}, and are checked for updating or removal on subsequent key
presses until such time as they are \emph{committed} (e.g.~when the user
explicitly moves the cursor). One of the main challenges with automatic
language boxes is to determine what text to reparse: we present several sub-heuristics (including

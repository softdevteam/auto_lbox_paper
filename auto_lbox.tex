...
location using that language's lexer and parser. It stops consuming text when
the language's parser reaches an error state, which means that no more input
can be consumed.
...
\end{center}
\begin{picture}(0,0)
    \put(-242,40){\textcolor{black}{\textbf{(a)}}}
    \put(-5,40){\textcolor{black}{\textbf{(b)}}}
\end{picture}
\vspace{-1.2em}
...
expressions wherever Java expressions are valid. \textbf{a)} The user inserted an SQL statement
into the program, but hasn't finished typing yet. However, since the \qtt{+} is invalid
in the SQL statement, we can already insert an SQL language box at that point. \textbf{b)} When
inserting the SQL language box, an error appears at \qtt{\}}, since the user hasn't finished
typing the Java expression. Parsing further than \qtt{+} to confirm the candidate would
thus lead to its false rejection.}
...
\caption{An example showing how using the first token, or first non-whitespace
  ...
  is a candidate for the same reason. `1 or 2 and 3;` is a candidate, because it following is a newline
    which can also be shifted after the language box, confirming it.}\laurie{ok, this example is going to confuse readers because the pasting thing feels weird. can we find a simpler example that triggers just with normal typing?}
    \lukas{I already thought about this. I don't think we can. Because if there are multiple possible
    insertions, the first one will always be immediately inserted after it has been typed, so we will never get multiple options this way.}
The

...
\begin{figure}
    ...
    \caption{Automatic language boxes in action with a composition of SQL
      ...
      putting their cursor immediately after `\texttt{a}' and typing `\texttt{,c\textasciicircum3}':
      ...
      puts the cursor immediately after \texttt{a} \laurie{we need to use a
      variable other than `a', because otherwise it's confused with a) b) c)
    etc.}. \textbf{b)} After typing `,', the SQL \texttt{SELECT a, FROM b} causes
    a syntax error. At this point, the automatic language box heuristic
    is used: by inserting a Java language box around \texttt{FROM} (a valid
    ...
    valid in the outer language again\laurie{this is the loose/fixed thing: how do we decide when to rerun the parser? is it just because ``a,b, FROM'' is syntactically invalid?}. \textbf{d)} Typing `\texttt{\textasciicircum3}' produces an error
...
Appluing these constraints to the example above, the inserted box would be

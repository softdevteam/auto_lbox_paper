...
possibility of old syntax errors being considered a second time.\laurie{i
...
does each new keypress make an old error back into a new error?}


\subsection{The candidates heuristic}

Once the consideration heuristic has triggered, we then have to search for
plausible language boxes to insert -- what we call the \emph{candidates heuristic}. The
...
language types. In this subsection we define, in no particular order, three
...
\laurie{lukas: can you flesh out rough descriptions of these?}
The first candidates heuristic is the \emph{stack} approach.

The second candidates heuristic is the \emph{history} approach.

The third candidates heuristic is the \emph{history} approach.
...
language boxes to insert. If there are zero candidates, then there is no more
work to be done. If one or more candidates have been found, we then need to see
...
if the insertion of a language box causes further syntax errors. Performance is
not a particular concern for the filtering heuristic, because the incremental
...
A first attempt at a filtering heuristic is to accept all candidate language
boxs which do not lead to a syntax error at the next non-whitespace token
\laurie{do we care about a syntax error before the language box?}. In other
words, if a language box is syntactically valid at its current point, we don't
mind if it causes subsequent errors, since we assume those are the result of an
incomplete program. The reason that we specify that the first non-whitespace
...
This means that the incremental parse nearly always inserts a (possibly empty)
...
language boxes to insert. If there are zero candidates, then there is no more
work to be done. If there is one candidate, we directly apply it to the user's
...
of marking to make sure that we don't insert the same language box again?}.

However, if there are multiple candidates, then we have two choices: we could
apply one of the candidates and present the others to the users as choices; or
simply to present all the candidates to the users as choices without applying
any of them. The former approach is surprisingly hard to do well. If were to
non-deterministically select one candidate and apply it, the user would be
...
as options to the user from which they must choose one. As we shall see in
Section~\ref{sec:evaulation}, this happens in sufficiently few cases for this
to be a significant usability problem. It is also worth noting that this is an
example of a fundamental difference between traditional batch parsing and
language boxes as part of incremental parsing: it is entirely feasible for us
to ask the user for their help in choosing language boxes as that choice is
made only once, not on each (batch) parse.

\laurie{the rest of the stuff in this section needs putting in a figure or similar}

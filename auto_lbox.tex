...
up to the root is not always possible}\laurie{can you explain this a bit more for me? detached in what way? i thought everything was attached, but perhaps attched to the wrong node? clearly i'm wrong in some way!} We thus use the
...
want to optimise memory use storing only the diffs of changed tokens values might be a better target.}\laurie{hang on, do we store the parsing stack or just a single parsing state? if it's the latter, fine (it's just a machine word at worse).}, we can just use the

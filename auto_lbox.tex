...
expression contained several subexpressions. For PHP and Java \laurie{why not lua too?}\lukas{The extracted lua expressions didn't suffer as much from the same program, so I left them as is. This can be changed tho.}\laurie{it'll confuse readers less if we're consistent -- unless it reduces the number of lua expressions to a ludicrously low amount}
